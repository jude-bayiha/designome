...
3. For tables and lists, extract row and column anatomy, alignment, density, hierarchy, actions, selection, grouping, sorting, filtering, pagination, expansion, batch action, empty, long-content, and small-screen implications.
   Preserve readable column labels, their semantic roles and order, grouping depth, fully and partly visible row counts, and occupied versus empty space within the admitted region. Mark cropped or unreadable entries unknown. Visible row count describes this capture, not total data volume. A reconstruction must not silently replace funding with revenue, creation date with employee count, or a dense collection with a sparse sample; record deliberate content changes separately from source reproduction.
