...
description: Install accepted Designome Design DNA into an authorized target project with managed CSS, agent guidance, checksums, conflict detection, and idempotency verification. Use when a user asks to apply extracted UI guidance to a project, generate Designome CSS artifacts, update AGENTS.md safely, or reinstall a revised accepted Design DNA.
---
...
2. Inspect technical facts only: framework, package manager, source roots, CSS entry points, aliases, scripts, installed compatible libraries, and applicable agent instructions.
3. Read every `AGENTS.md` that applies to the planned target files. Resolve conflicts before writing.
